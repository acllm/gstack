...
_CONTRIB=$(~/.trae/skills/gstack/bin/gstack-config get gstack_contributor 2>/dev/null || true)
...
for _PF in ~/.gstack/analytics/.pending-*; do [ -f "$_PF" ] && ~/.trae/skills/gstack/bin/gstack-telemetry-log --event-type skill_run --skill _pending_finalize --outcome unknown --session-id "$_SESSION_ID" 2>/dev/null || true; break; done
```

If `PROACTIVE` is `"false"`, do not proactively suggest gstack skills — only invoke
them when the user explicitly asks. The user opted out of proactive suggestions.

If output shows `UPGRADE_AVAILABLE <old> <new>`: read `~/.trae/skills/gstack/gstack-upgrade/SKILL.md` and follow the "Inline upgrade flow" (auto-upgrade if configured, otherwise AskUserQuestion with 4 options, write snooze state if declined). If `JUST_UPGRADED <from> <to>`: tell user "Running gstack v{to} (just updated!)" and continue.
...
If A: run `~/.trae/skills/gstack/bin/gstack-config set telemetry community`
...
If B→A: run `~/.trae/skills/gstack/bin/gstack-config set telemetry anonymous`
If B→B: run `~/.trae/skills/gstack/bin/gstack-config set telemetry off`
...
1. **Check for a prior override on this branch:**
   ```bash
   source <(~/.trae/skills/gstack/bin/gstack-slug 2>/dev/null)
...
2. **If no override exists,** use AskUserQuestion:
   ...
   - For Design Review: run `source <(~/.trae/skills/gstack/bin/gstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
1. Read `.trae/skills/gstack/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
```bash
source <(~/.trae/skills/gstack/bin/gstack-diff-scope <base> 2>/dev/null)
```
...
2. **Read `.trae/skills/gstack/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
~/.trae/skills/gstack/bin/gstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
Read `.trae/skills/gstack/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps.
...
```bash
~/.trae/skills/gstack/bin/gstack-review-log '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE"}'
```
...
Read `.trae/skills/gstack/review/TODOS-format.md` for the canonical format reference.
